var GLOBAL_URL = "http://218.80.250.92:56679/healthy_service/healthy/dispatchV2";
//var GLOBAL_URL = "https://mobile.e-yao.com.cn/healthy_service/healthy/dispatchV2";
//图片加载失败时默认图片
...
var classify_URL = "http://218.80.250.92:56679";
//版本号
var app_version = "V1.0.0";
//跳转到jjk的路径
//var jumpTojjk = "https://jjk.wdieb.com/employeeBenefits/rest/goldenHealth";
var jumpTojjk = "https://test.wdieb.com/employeeBenefits/rest/goldenHealth";
/**
 * 打开员福云原生页面
...
window.confirm2 = function(msg,callback1, callback2) {
	divShow1({
		strTitle: '提示',
		tip: msg,
		content1:"绑定",
...
window.confirm1 = function(msg,callback1, callback2) {
	divShow1({
		strTitle: '提示',
		tip: msg,
		content1:"登录",
...
//actionSheet确认框
window.actionSheet = function(json,callback1,callback2){
	if(window.plus){
		...
		var cf = window.confirm(json.title);
		if(cf){
			eval(callback1);
		} else {
			if(callback2){
...
function autoDivShow(opts) {
	...
	var h = ($(window).height() - $(".autoDSMsg").height()+40) / 2;
	$(".autoDSMsg").css({
		'top': h,
		'left': w
	});
	$(".autoDSMsg").fadeIn();
	setTimeout('$(".autoDSMsg_' + adsIndex + '").fadeOut("slow").remove();', time * 1000);
	adsIndex++;
}
window.toast1 = function(msg, closeTime) {
	autoDivShow1({
		msg: msg,
		closeTime: closeTime, //时间单位s
	});
...
//toast框，自动消失
var adsIndex1 = 0;
function autoDivShow1(opts) {
	var msg = opts.msg,
		time = (opts.closeTime) || 5;
	//移除其他提示消息
	$(".autoDSMsg1").remove();
	var autoDivShow = '<div class="autoDSMsg1 autoDSMsg1_' + adsIndex1 + '">' + msg + '</div>';
	$('body').append(autoDivShow);
	var w = ($(window).width() - $(".autoDSMsg1").width() - 20) / 2;
	var h = ($(window).height() - $(".autoDSMsg").height()+40) / 2;
	$(".autoDSMsg1").css({
		'top': h,
		'left': w
	...
	$(".autoDSMsg1").fadeIn();
	setTimeout('$(".autoDSMsg1_' + adsIndex1 + '").fadeOut().remove();', time * 1000);
	adsIndex1++;
}
function divShow1(opts) {
	if(opts.tip == undefined || opts.tip == null || opts.tip == ""){
		var tip = "请求不到数据，请检查网路;";
	}else{
		var tip = opts.tip.toString();
	};
	if(opts.content1 == undefined || opts.content1 == null || opts.content1 == ""){
		var  content1= "确认";
	}else{
		var content1 = opts.content1.toString();
	};
	if(opts.content2 == undefined || opts.content2 == null || opts.content2 == ""){
		var  content2= "确认";
	}else{
		var content2 = opts.content2.toString();
	};
	...
	msgDiv.style.borderRadius = 15+"px";
	...
	title.innerHTML = "<table border='0' width='100%' height='100%'><tr><td align='center' style='border:none;'>" + opts.strTitle + "</td></tr></table>";
	...
	var txtHeight = dialogHeight - 40 + "px";
	...
	if (icon) {
		if (icon == "failed") {
			txtHTML = "<div style='width:" + dialogWidth + "px;height:" + txtHeight + ";word-break: break-all;word-wrap: break-word;'><div style='text-align:center;height:55%;width:90%;margin-left:5%;font-size:14px;color:#000000;'><div class='vAlign'><img src='" + basePath + "/images/alert.png' style='margin-right:5px;'/>" + tip +
				"</div></div><div style='text-align:center;width:100%;height:45%;'>";
		} else if (icon == "success") {
			txtHTML = "<div style='width:" + dialogWidth + "px;height:" + txtHeight + ";word-break: break-all;word-wrap: break-word;'><div style='text-align:center;height:55%;width:90%;margin-left:5%;font-size:14px;color:#000000;'><div class='vAlign'><img src='" + basePath + "/images/success.png' style='margin-right:5px;'/>" + tip +
				"</div></div><div style='text-align:center;width:100%;height:45%;'>";
		}
	} else {
		txtHTML = "<div style='width:" + dialogWidth + "px;height:" + txtHeight + ";word-break: break-all;word-wrap: break-word; '><div style='text-align:center;height:55%;width:90%;margin-left:5%;font-size:14px;color:#000000;'><div class='vAlign'>" + tip +
			"</div></div><div style='text-align:center;width:100%;height:45%;'>";
	}
	if (type) {
		txtHTML += "<button id='dialogConfirmButton' class='dialogButton' type='button'>"+content1+"</button><button id='dialogCloseButton' style='border-bottom-right-radius:15px;' class='dialogButton' type='button'>"+content2+"</button>";
	} else {
		txtHTML += "<button id='dialogCloseButton' class='dialogCloseButton' style='border-bottom-left-radius:15px;border-bottom-right-radius:15px;font-weight:500;-webkit-border-bottom-left-radius:15px;-webkit-border-bottom-right-radius:15px;color:#31AEFD;' type='button'>确定</button>";
	}
	txtHTML += "</div></div>";
